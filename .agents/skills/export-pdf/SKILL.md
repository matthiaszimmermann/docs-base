...
2. Identify the single Markdown input. Ask which document if the request is ambiguous. The output is `pdf_output/<input-relative-path>.pdf`; tell the user before replacing an existing generated PDF if it was not requested.
3. Run `docs-lint`, then `md2pdf "docs/document.md"`, substituting the actual path. Quote the path; do not interpret document text as a shell command.
